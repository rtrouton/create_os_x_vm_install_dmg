...
2. A 10.7.x, 10.8.x, 10.9.x, or 10.10.x installer from Apple's Mac App Store.
...
If you have a 10.10.x Yosemite installer available, run this command:

sudo /path/to/create_vmware_osx_install_dmg.sh "/Applications/Install OS X Yosemite.app" /path/to/output_directory

This should produce a DMG file at output_directory that's named OSX_InstallESD_10.10_14A389.dmg. An MD5 checksum is printed at the end of the process.
...
3. Additional installer packages can be added using a first boot package, which is added to 
   ...
   distribution-style flat package that can be used with 10.7 / 10.8 / 10.9 / 10.10 OS installs.
...
This script has been tested with Apple's 10.7.5, 10.8.5, 10.9.5, and 10.10.0 installers from the Mac App Store.
...
In my testing, 10.7.x, 10.8.x and 10.10.x disk images will successfully install a recovery partition into the VM.